...
import { deleteArtifact, getAllArtifacts } from "../../../api/ArtifactAPI";
...
export default function ArtifactTable() {
  ...
  const confirmDelete = async (id) => {
    try {
      ...
      setArtifact(artifact.filter((n) => n.id !== id));
      setToastBg("success");
  ...
  return (
    <>
      <div className="manage-artifact-table">
        <ArtifactSearch onSearch={handleSearch} />
        <div
          className="btn btn-primary "
          style={{ "margin-left": "10px" }}
          onClick={() => navigate("/admin/artifacts/create")}
        >
          Thêm mới
        </div>
              ...
              {artifact.map((n) => (
                <tr key={n.id}>
                  <th scope="row" style={{ textAlign: "center" }}>
                    <NavLink to={`/artifacts/${n.id}`}>{n.id}</NavLink>
                  </th>
                  <td>{n.name}</td>
              ...
              <div className="modal-content">
                ...
                <div className="modal-footer">
                  ...
                  <button
                    type="button"
                    className="btn btn-danger"
                    onClick={() => confirmDelete(artifactToDelete)}
                  >
                    Xóa
                  </button>
                </div>
